...
sqlt --from DBI --dsn dbi:Pg:dbname=omniscience --db-user omniscience_user \
  --to YAML > schema/schema.yaml
# Create a diagram from the YAML file.
sqlt-graph --from=YAML --color --show-datatypes --show-sizes \
  --show-constraints --output-type png schema/schema.yaml > schema/schema.png
